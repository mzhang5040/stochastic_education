...
for i, x in enumerate(xs):
    ...
    axA.text(x, y - 0.55, sub[i], ha="center", va="top", fontsize=8.6, color="#222")
...
axA.text(2.0, y + r + 0.62, "self-loops: persistence $p_{ii}$",
         ha="center", fontsize=9.2, color="#222")
...
axA.text(0.5, 0.52, "adjacent up", color="#1f4e79", fontsize=9.2, ha="center")
axA.text(3.5, -0.52, "adjacent down", color="#5a5a5a", fontsize=9.2, ha="center")
...
axA.text(2.0, 1.28, "non-adjacent moves: dashed, weight $\\propto\\rho^{|j-i|-1}$ "
         "($\\rho=0.7$)", ha="center", fontsize=9.2, color="#555")
axA.set_title("(A) Calibrated five-state achievement chain", fontsize=10, pad=4)
...
axB.set_ylabel("leverage  $L_i = \\pi_i\\,\\pi_5\\,m_{i5}$", fontsize=9.5)
