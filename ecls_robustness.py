...
every ECLS robustness number reported in the paper:
    ...
    Sec 3.3.1  cross-state SES-slope difference tests (JK2);
               base-year (broader-cohort) weight robustness for the two tail slopes;
               grade-interval (wave-pair) fixed-effects check;
               attrition check (mean SES of s5-origin observed vs censored)
    Sec 3.3.2  jump-size regression gamma1, base-year and complete-panel weight
    Sec 3.3.3  threshold decomposition (pooled vs within-wave, unweighted vs
...
# === Sec 3.3.1  cross-state difference tests ===============================
def slope_diff_jk(ka, kb):
    d0 = E.state_slope(df, E.PRIMARY, ka) - E.state_slope(df, E.PRIMARY, kb)
    reps = np.array([E.state_slope(df, r, ka) - E.state_slope(df, r, kb) for r in E.PRIMARY_REPS])
    se = E.jk2_se(d0, reps)
    return d0, se, d0 / se

print("=" * 66)
print("SEC 3.3.1 - cross-state SES-slope difference tests (JK2)")
print("=" * 66)
...
print("\n  base-year weight robustness, broader cohort (tails):")
for k, lab in [(0, "s1 (floor)"), (4, "s5 (ceiling)")]:
    b0 = E.state_slope(df, E.SENSITIVITY, k)
    se = E.jk2_se(b0, [E.state_slope(df, r, k) for r in E.SENSITIVITY_REPS])
...
thr = E.wave_thresholds(df, E.PRIMARY)
obs, cen = [], []
for a, b in E.PAIRS:
...
# === Sec 3.3.2  jump-size regression =======================================
def jump(T):
    e = T[(T["fr"] == 0) & (T["to"].isin([1, 2]))]
    return E.wlogit_slope((e["to"] == 2).values.astype(float), e["ses"].values, e["w"].values), len(e)

print("\n" + "=" * 66)
print("SEC 3.3.2 - jump-size regression gamma1 (s3 vs s2 | exit s1)")
print("=" * 66)
g0, n0 = jump(T0)
greps = []
for r in E.PRIMARY_REPS:
...
print(f"  complete-panel: gamma1 = {g0:+.3f}  JK SE = {gse:.3f}  z = {g0/gse:+.1f}  "
      f"CI=[{g0-1.96*gse:+.3f},{g0+1.96*gse:+.3f}]  n={n0:,}")
gp, _ = jump(E.transitions_for_weight(df, E.SENSITIVITY))
print(f"  base-year:      gamma1 = {gp:+.3f}  -> robustly null")

# === Sec 3.3.3  threshold decomposition ====================================
...
print("\n" + "=" * 66)
print("SEC 3.3.3 - gamma1 by mobility definition (pooled vs within-wave)")
print("=" * 66)
pooled = np.percentile(pd.concat([df[w].dropna() for w in E.WAVES]).values, [25, 40, 60, 80])
thr_pooled = {wv: pooled for wv in E.WAVES}
thr_wave = E.wave_thresholds(df, E.PRIMARY)

def decomp(thrd, weighted, require_pos):
    T = E.build_transitions(df, thrd, E.PRIMARY, require_pos=require_pos)
    e = T[(T["fr"] == 0) & (T["to"].isin([1, 2]))]
